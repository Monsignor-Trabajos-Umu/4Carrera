...
class Aruco_Master(object):
    ...
    def __init__(self, MarkerX_cols, MarkerY_rows, marker_length=0.05, space_bet_marker=0.001, dictionary=None,
    ...
    def draw_axis_on_board(self):
        ...
        while True:
            ...
            gray = im.gray(img)
            corners, ids, rejectedImgPoints = aruco.detectMarkers(gray, self.aruco_dict, parameters=self.parameters)
            corners1, ids1, rejectedImgPoints1, rec_index = aruco.refineDetectedMarkers(gray, board, corners, ids,
                                                                                        rejectedImgPoints)
            getcount = self.get_marker_count(ids1)
            if getcount > 0:
                ret, rv, tv = aruco.estimatePoseBoard(corners1, ids1, board, cam_matrix, dist_coeff)
    ...
    def calibrate_camera_aruco_init(self, output_samples_dir, number_of_samples_to_take):
        im = Imutils_Master("http://192.168.2.72/live")  # change your video source here.
        img = im.getframe()
