    ...
    while True:
        ...
        if option == 1:
            ...
            while firstFrame is None:
                ...
                cv.putText(gray_copy_text, "Pulsa f para capturar", (10, 20),
                           cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                cv.imshow('fondo', gray_copy_text)
                key = cv.waitKey(1) & 0xFF
                if key == ord('f'):
                    firstFrame = gray
                    cv.imwrite(PATH_FONDO, firstFrame)
                    cv.destroyWindow('fondo')

        elif option == 2:
            if firstFrame is None:
                firstFrame = cv.imread(PATH_FONDO)
            if firstFrame is None:
                print(f"{Fore.RED} Seleciona captura el fondo antes guapo")
                continue

            cv.namedWindow("Camara")
            cv.createTrackbar("Sensiviliad contornos", "Camara", 0, 10000, DoNothing)
            while True:
                estado_moviento = "ALL CLEAR"
                gray_actual, color_actual = cap_img()
                # Calculamos los contornos
                contours = capture_contours(firstFrame, gray_actual)
