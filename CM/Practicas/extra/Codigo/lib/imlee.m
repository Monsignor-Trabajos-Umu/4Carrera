function [X, Xamp, tipo, m, n, mamp, namp, TO] = imlee(fname)
    ...
    %  m: Alto original de la imagen (n de filas)
    %  n: Ancho original de la imagen (n de columnas)
    %  mamp: Alto ampliado de la imagen (n de filas)
    %  namp: Ancho ampliadode la imagen (n de columnas)
    %  TO: Tama�o original de archivo para calcular RC
    ...
    % Lee info de imagen
    s = imfinfo(fname);
    TO = s.FileSize; % Tama�o original

    % Carga archivo, convierte a RGB y calcula tama�o
    ...
    [m n plan] = size(X); % tama�o original
    ...
    mamp = m + mpad; % Alto ampliado de la imagen (n de filas)
    namp = n + npad; % Ancho ampliadode la imagen (n de columnas)
